...
mkdir_datos() {
    if [[ ! -d "$HOME/.local/data" ]]
    then
        sudo mkdir ~/.local/data
        sudo chown "$USER" ~/.local/data
        sudo chmod 750 ~/.local/data
    else
        echo "The mount point ~/.local/data has already been made"
    fi
}
...
# ~/.local/data
UUID=$(sudo blkid -s UUID -o value /dev/mapper/cryptdata)      ~/.local/data        btrfs     	    defaults,noatime,autodefrag,compress-force=zstd:1,space_cache=v2,subvol=/@	0 2
EOT
}
...
case "${1}" in
    ...
    mount)
        mkdir_datos
        sudo cryptsetup luksOpen /dev/sda1 cryptdata
        sudo mount -o defaults,noatime,autodefrag,compress-force=zstd:1,space_cache=v2,subvol=@ /dev/mapper/cryptdata ~/.local/data
        sudo hdparm -S 60 /dev/sda
        ;;
    unmount)
        sudo umount ~/.local/data
